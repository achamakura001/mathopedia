...
from app.api.models import (
    ...
    achievements_response_model,
    topic_breakdown_model,
    daily_progress_model,
    error_model
)

api = Namespace('profile', description='User profile and performance operations')

@api.route('/')
class ProfileResource(Resource):
    @api.doc('get_profile')
    @api.marshal_with(profile_response_model)
    @api.response(200, 'Success', profile_response_model)
...
@api.route('/daily-progress')
class DailyProgressResource(Resource):
    @api.doc('get_daily_progress')
    ...
    @api.response(401, 'Unauthorized', error_model)
    @api.response(400, 'Invalid date format', error_model)
    @jwt_required()
    def get(self):
...
class AchievementsResource(Resource):
    @api.doc('get_achievements')
    @api.marshal_with(achievements_response_model)
    @api.response(200, 'Success', achievements_response_model)
    @api.response(401, 'Unauthorized', error_model)
    @jwt_required()
    def get(self):
...
@api.route('/grade/<string:grade_level>/topics')
class GradeTopicPerformanceResource(Resource):
    @api.doc('get_grade_topic_performance')
    @api.marshal_with(topic_breakdown_model)
    @api.response(200, 'Success', topic_breakdown_model)
    @api.response(401, 'Unauthorized', error_model)
    @api.response(404, 'Grade not found', error_model)
    @jwt_required()
    def get(self, grade_level):
        ...
        if not topics:
            api.abort(404, f'No topics found for grade level: {grade_level}')
